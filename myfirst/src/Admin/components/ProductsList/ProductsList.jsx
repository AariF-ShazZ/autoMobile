...
const ProductsList = ({type}) => {
  ...
  return (
    <>
      <Flex
      display={{ base: { direction: "column" }, md: { direction: "column" }, lg: "flex" }}>
      {
  type !== "admin" ? (
    <div>
    ...
    <Box  w={"5%"}>
      <Box display={{ base: 'inline', md: "inline", lg: "inline" }} alignItems={"center"} justifyContent={"flex-end"}>
        {/* <Button onClick={onOpen} color={"#fff"} bg={"#ff0000"} m={"2% 0 2% 60% "}>
          Filter Here
        </Button> */}
        <BsFilterRight fontSize={"40px"} onClick={onOpen} color={"#ff0000"} m={"2% 0 2% 60% "} cursor={"pointer"}/>
        <Modal finalFocusRef={finalRef} isOpen={isOpen} onClose={onClose}>
          <ModalOverlay />
          <ModalContent bg={"#fff"}>
            <ModalHeader color={"gray"}>Filter Modal</ModalHeader>
      ...
      <Flex  alignItems={"center"} justifyContent={"center"}>
        <Box display={"flex"} w={"15%"} alignItems={"center"} justifyContent={"space-between"} sx={{ margin: "30px 0px" }}>
          ...
          <Button onClick={() => setPage((prev) => prev - 1)} colorScheme='#f00' bg={"#f00"} color={"#fff"} isDisabled={page == 1}>Prev</Button>
